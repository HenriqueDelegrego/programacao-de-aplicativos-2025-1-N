...
public class Ex7 {
	...
	public static void main(String[] args) {
		...
		Aeronave a1 = new Aeronave("Boeing", 200, 500, 1000, 5);
		...
		for (int i = 0; i < vetor.length; i++) {
			if (vetor[i].qtdPassageiros > maisPassageiros) {
				maisPassageiros = vetor[i].qtdPassageiros;
				modeloMaisPassageiros = vetor[i].modelo;
			}
		}
		...
		for (int i = 0; i < vetor.length; i++) {
			if (vetor[i].calcularAutonomia() > maiorAutonomia) {
				...
				modeloMaiorAutonomia = vetor[i].modelo;
			}
		}
		...
		for (int i = 0; i < vetor.length; i++) {
			if (vetor[i].calcularDistanciaMaxima() > maiorDistancia) {
				...
				modeloMaiorDistanica = vetor[i].modelo;
			}
		}
